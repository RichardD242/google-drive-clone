...
import { deleteFolder, restoreFolder } from "@/lib/actions/folder.actions";
import { usePathname } from "next/navigation";
import { Button } from "@/components/ui/button";

const TrashFolderActionDropdown = ({ folder }: { folder: FolderDocument }) => {
        ...
        <DialogHeader className="flex flex-col gap-3">
          ...
          {action.value === "restore" && (
            <p className="delete-confirmation">
              restore <span className="delete-file-name">{folder.name}</span>{" "}
              from trash?
            </p>
          )}
          {action.value === "delete" && (
            <p className="delete-confirmation">
              ...
              <span className="delete-file-name">{folder.name}</span> permanently?
            </p>
          )}
        </DialogHeader>
...
export default TrashFolderActionDropdown;
